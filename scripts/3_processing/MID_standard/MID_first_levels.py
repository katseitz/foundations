...
def finish_preproc(sub, ses, run, funcindir, sesoutdir):
    ...
    file_mid = os.path.join(funcindir, [x for x in flist if ('_space-MNI152NLin6Asym_desc-preproc_bold.nii.gz' in x and 'task-mid_run-0' + run in x)][0])
    ...
    param_mid_file = open(os.path.join(funcindir, sub+'_ses-1_task-mid_run-0' + run + '_space-MNI152NLin6Asym_desc-preproc_bold.json'),)
...
def extract_rois(sub, ses, funcindir, sesoutdir):
    ...
    #load tr
    param_mid_file = open(os.path.join(funcindir, sub+'_ses-1_task-mid_run-01_space-MNI152NLin6Asym_desc-preproc_bold.json'),)
    param_mid_df = json.load(param_mid_file)
    # Get TRs
    tr = param_mid_df['RepetitionTime']
        ...
        if(("sub-f12" in sub.name) and not(".html" in sub.name)):
            ...
            if(os.path.exists(preproc_rest)):
                ...
                try:
                    ...
                    print(e)
    with open('foundations_MID_all_TRs_09102024.csv', 'a') as myfile:
        wr = csv.writer(myfile)
        for row in tr_counts:
            wr.writerow(row) 
    
    with open('foundations_MID_all_ROIs_09102024s.csv', 'a') as myfile:
        wr = csv.writer(myfile)
